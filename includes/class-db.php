...
class WP_Psycho_DB {
    ...
    public static function install() {
        ...
        dbDelta( "CREATE TABLE {$p}questions (
            id bigint(20) NOT NULL AUTO_INCREMENT,
            test_id bigint(20) NOT NULL DEFAULT 0,
    ...
    public static function get_tests( $status = null ) {
        ...
        if ( $status !== null ) {
            return $wpdb->get_results( $wpdb->prepare(
                "SELECT * FROM {$p}tests WHERE status = %d ORDER BY id DESC",
                $status
            ) );
        }
        return $wpdb->get_results( "SELECT * FROM {$p}tests ORDER BY id DESC" );
    }
    ...
    public static function get_result( $result_id ) {
        ...
        return $wpdb->get_row( $wpdb->prepare(
            "SELECT r.*, p.name AS participant_name, p.email AS participant_email,
             ...
             FROM {$p}results r
             LEFT JOIN {$p}participants p ON r.participant_id = p.id
             LEFT JOIN {$p}tests t ON r.test_id = t.id
             WHERE r.id = %d",
            $result_id
        ) );
    }
