...
class WP_Psycho_Scoring_Engine {
    ...
    public function submit_test() {
        ...
        if ( ! $session_key ) {
            wp_send_json_error( [ 'message' => __( 'Session expired. Please refresh and try again.', 'wp-psycho' ) ] );
        }

        $participant = WP_Psycho_DB::get_participant( $session_key );
        if ( ! $participant ) {
            wp_send_json_error( [ 'message' => __( 'Participant not found.', 'wp-psycho' ) ] );
        }
        ...
        if ( ! $attempt_id ) {
            wp_send_json_error( [ 'message' => __( 'Could not record attempt.', 'wp-psycho' ) ] );
        }
        ...
        foreach ( $responses as $question_id => $option_id ) {
            ...
            $option = $wpdb->get_row( $wpdb->prepare(
                "SELECT o.score, q.trait FROM {$p}options o
                 LEFT JOIN {$p}questions q ON o.question_id = q.id
                 WHERE o.id = %d AND o.question_id = %d",
                $option_id,
                $question_id
            ) );

            $score = $option ? intval( $option->score ) : 0;
            $trait = $option ? sanitize_text_field( $option->trait ) : '';
        ...
        if ( ! $result_id ) {
            wp_send_json_error( [ 'message' => __( 'Could not save result.', 'wp-psycho' ) ] );
        }
